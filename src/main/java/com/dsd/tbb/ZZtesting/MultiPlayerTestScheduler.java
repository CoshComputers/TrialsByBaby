...
public class MultiPlayerTestScheduler {
    ...
    private static int maxThreshold = 70; // Maximum threshold value
    private static boolean isEnabled = false;
    public static void onTick() {
        if(!isEnabled) return;
        ConfigManager configManager = ConfigManager.getInstance();
        TrialsConfig trialsConfig = configManager.getTrialsConfig();
        int currentThreshold = trialsConfig.getSpawnGiants(); //baby spawn
        tickCount++;
        ...
        if (tickCount >= INCREMENT_INTERVAL) {
            if (currentThreshold < maxThreshold) {
                ...
                trialsConfig.setSpawnGiants(newThreshold);

                TestEventLogger.logEvent("380df991-f603-344c-a090-369bad2a924a","Giant Zombie Spawn Increase","TESTTICKER","Increasing to " + newThreshold);
